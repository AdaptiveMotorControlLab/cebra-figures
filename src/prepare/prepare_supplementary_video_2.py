...
ax1_np.imshow(
    np_neural.T.reshape(1000, 900, -1).mean(axis=-1)[:50, :],
    aspect="auto",
    cmap="gray",
    vmin=0,
    vmax=1.5,
...
cb = mpl.colorbar.ColorbarBase(cb_ax, cmap=cm.magma, norm=norm)
...
def animate_position(i):
    ...
    ## For NP trace
    np_points = np_test_smooth[
        start_idx * frame_ratio : (start_idx + i * rate) * frame_ratio, :3
    ][np.arange(0, i * rate * frame_ratio, frame_step)].reshape(-1, 1, 3)
    np_seg = np.concatenate([np_points[:-1], np_points[1:]], axis=1)
    np_coll = Line3DCollection(
        np_seg,
        colors=colors_np_test[
            start_idx * frame_ratio : (start_idx + i * rate) * frame_ratio
        ][np.arange(0, i * rate * frame_ratio, frame_step)],
        linewidth=3,
    )
...
ani = animation.FuncAnimation(fig, animate_position, 900, interval=50)
