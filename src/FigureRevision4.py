...
for m in modality:
    ...
    for i in range(9):
        ...
        if 'np' in m:
            ...
            label = np.arange(900)
        ax.scatter(viz[m][num_neurons[i]][:,0],\
                    viz[m][num_neurons[i]][:,1], s =1,  c = np.tile(label,10), cmap = 'magma')
...
for i,d in enumerate(dims):
    ...
    ax.errorbar(np.arange(10),mean,err,  label = d, color = 'black', alpha =(i+1)*0.1, markersize= 20, linewidth = 8)
...
plt.legend(fontsize= 30, loc = 'lower right', title= 'Dimension', title_fontsize = 20, bbox_to_anchor=(1.2,0)  )
...
make_heatmap_from_df(pd.DataFrame(data['cortices_consistency']['joint_cortices']), 'Joint Ca-Joint NP', 100,70)


make_line_strip_from_df(data['cortices_consistency']['joint_v1'], 'V1 inter-intra', 50,100)
