...
# # Figure S6: Hypothesis testing with CEBRA
...
velocity = scipy.signal.savgol_filter(
    dataset.index[:, 0], window_length=31, polyorder=2, deriv=1
)
accel = scipy.signal.savgol_filter(
    dataset.index[:, 0], window_length=11, polyorder=2, deriv=2
)
...
plt.plot(dataset.index[:, 0], velocity, linewidth=0.5, color="black", alpha=0.8)
plt.xlabel("position")
plt.ylabel("velocity")
...
options = itertools.chain.from_iterable(
    (itertools.product(("none",), options), itertools.product(options, ("none",)))
)
